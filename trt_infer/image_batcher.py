...
class ImageBatcher:
    ...
    def preprocess_image(self, image_path):
        ...
        * V1: The algorithm for EfficientNet V1, aka "Legacy", as defined in automl/efficientnetv2/preprocess_legacy.py.
        ...
        pad = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        if self.preprocessor == "V1MS":
            ...
            pad = cv2.resize(image, (128,128))
            pad = np.asarray(pad, dtype=self.dtype)
            pad = pad - np.asarray([123.68, 116.28, 103.53])
            pad = pad / np.asarray([58.395, 57.120, 57.375])
        else:
            print("Preprocessing method {} not supported".format(self.preprocessor))
            sys.exit(1)
        if self.format == "NCHW":
            pad = np.transpose(pad, (2, 0, 1))
        return image, pad
    ...
    def get_batch(self):
        ...
        for i, batch_images in enumerate(self.batches):
            ...
            for i, image in enumerate(batch_images):
                self.image_index += 1
                src_data, batch_data[i] = self.preprocess_image(image)
            self.batch_index += 1
            yield src_data, batch_data, batch_images
