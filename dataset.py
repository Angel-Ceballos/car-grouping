...
class CarDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            color = self.colors.get(self.data.iloc[index]['color'])
            c_type = self.types.get(self.data.iloc[index]['type'])
            orientation = self.orientations.get(self.data.iloc[index]['orientation'])
            labels = np.array([color, c_type, orientation])
...
if __name__ == "__main__":
    ds = CarDataset(csv_file="/home/angel/computer_vision/car-grouping/labels/train_data.csv", train=True, transform=config.train_transforms)
